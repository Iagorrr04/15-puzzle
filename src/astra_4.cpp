...
struct State
{
    ...
    int calc_heuristic(){
        ...
        // pegar os conflitos por linha.
        int cl, cr;
        for (r = 0; r < N; r++) {
            for (cl = 0; cl < M; cl++) {
                for (cr = cl + 1; cr < M; cr++) {
                    if (r == pR[final_board[r*M+cl]] && pR[final_board[r*M+cl]] == pR[final_board[r*M+cr]] && pC[final_board[r*M+cl]] > pC[final_board[r*M+cl]]) {
                        conflicts++;
                    }
                }
            }
        ...
        // pegar os conflitos por coluna.
        int rU, rD;
        for (c = 0; c < M; c++) {
            for ( rU = 0; rU < N; rU++) {
                for (rD = rU + 1; rD < N; rD++) {
                    if (c == pC[final_board[rU*M+c]] && pC[final_board[rU*M+c]] == pC[final_board[rD*M+c]] && pR[final_board[rU*M+c]] > pR[final_board[rD*M+c]]) {
                        conflicts++;
                    }
                }
            }
...
int32_t main()
{
    ...
    for(i = 0; i < N*M; ++i) final_board[i]= i;
